...
⚠ **The USB1 host port moves that to ≈ 1.865 A** if a stick draws its full limit during a refresh —
still under the 2.0 A row below, but that is the whole remaining margin, and it is why the current
limit is set at 500 mA nominal rather than the 1 A or 1.5 A the part can do. Two mitigations, neither
requiring a change: the SoM's 1.0 A is a *design bound* against 651 mA measured under a load Glider
does not run, and firmware owns `USB1_DRVVBUS`, so it can hold the port off during a refresh if
bring-up shows it matters.
