...
Two things to do before the fab order, neither of which blocks placement:
one free gateware build to confirm the `-2` part closes timing (`docs/fpga.md` §1.1), and
**adding 2 × `C3646540` per board to the assembly BOM** — the two `BTH-060` receptacles are not
generated by any symbol and LCSC has 60 of them (`docs/som.md` §10). Per-sheet specs live in
